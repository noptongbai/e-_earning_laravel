...
class  Manage2Controller extends AdminsController {
	public function getIndex(){
		$user = Users::where('type','=',"student")->orderBy('id')->paginate(50);  

		
		return view('admin.manage2.index',['user'=>$user]);
	}
	...
	public function getForm($id = 0){
		...
		return view('admin.manage2.form',$data);
	}
	...
	public function getProfile(){
		$id = Auth::user()->id;
		$user = Users::where('id',$id)->first();
		if(!$user) return redirect('admin/student/index');
		$data = array('id' => $id,'user' => $user);
		return view('admin.manage2.form',$data);
	}
	...
	public function postAction(Request $request){
		...
		if($request->exists('delete')){
			...
			return view('admin.manage2.add',['data'=>$data],['subject'=>$subject]);
			}
			return redirect()->back();
		}
			...
			if($arr  && $user){
				...
				return redirect()->to('admin/student')->with('success','Successfully Added');
		}
			return redirect()->to('admin/student')->with('fail','Fail to Added select something wrong');
	}
