...
?>
        <input type="hidden" id="refreshed" value="no">
         <div class="row">
            <div class="col-md-12">
                    <h1>Update {{$gallery->name}} <a class="btn btn-default pull-right" href="{{url('/teacher/'.$name.'/upvideo/list')}}" title="Back" style="background-color: transparent;">Back</a></h1>  
            </div>
        </div>
        <form class="form" method="post" class="dropzone" id="addImages" action="{{url('/teacher/'.$name.'/upvideo/update/'.$gallery->id)}}">
        <div class="row">
            <div class="col-md-9">
                <div id="gallery-images">
                    <ul>
                        @if(isset($gallery))
                        <li  value="{{$gallery->id}}" name="video" id="img"><video src="{{url($gallery->images->file_path)}}" class="responsive-video" controls="controls" width="640" height="360"></video></li>
                        <h1></h1>
                        <h2><input type="hidden" name="oldimage" value="{{$gallery->images->id}}"></h2>
                        @endif
                    </ul>
                </div>
            </div>
